...
Variant of ``test_fake_flickr.py`` that additionally dumps per-image (generator,
image_id, label, score) rows to ``--per-image-csv``, needed to pair images across
the full-image and inpainting-background tables for a paired significance test
(e.g. Wilcoxon signed-rank). Writes to new output paths by default so it never
overwrites the original results.
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--results-csv", type=Path,
                   default=REPO_ROOT / "data" / "results" / "fake_flickr_aide_paired_check.csv",
                   help="Output CSV path (aggregate, one row per generator). "
                        "Defaults to a new file so the original results are never overwritten.")
    p.add_argument("--per-image-csv", type=Path,
                   default=REPO_ROOT / "data" / "results" / "fake_flickr_aide_per_image.csv",
                   help="Output CSV path for per-image (generator, image_id, label, score) rows.")
    p.add_argument("--seed", type=int, default=0,
                   help="Seed for torch/numpy/random. Inference is otherwise deterministic "
                        "(fixed weights, no augmentation, fixed item order); recorded for "
                        "the run metadata regardless.")
...
def main() -> None:
    ...
    for gen in args.generators:
        ...
        # `items`/dataset order == DataLoader(shuffle=False) output order, so
        # y_prob[i]/y_true[i] correspond 1:1 to items[i].
        for (path, label), prob in zip(items, y_prob):
            per_image_rows.append({
                "generator": gen, "image_id": Path(path).stem,
                "label": int(label), "score": float(prob),
            })

    args.per_image_csv.parent.mkdir(parents=True, exist_ok=True)
    with args.per_image_csv.open("w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=["generator", "image_id", "label", "score"])
        writer.writeheader()
        writer.writerows(per_image_rows)

    meta = {
        "script": "test_fake_flickr_paired.py",
    ...
    meta_path = args.per_image_csv.with_suffix(".meta.json")
    with meta_path.open("w") as f:
        json.dump(meta, f, indent=2)
    print(f"Per-image scores written to {args.per_image_csv}")
    print(f"Run metadata (seed, etc.) written to {meta_path}")
